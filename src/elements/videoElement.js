...
export default class VideoElement extends Element {
	constructor(quickxr, domElement, layer) {
		...
		this.quickxr.aframe.scene.appendChild(this.video360)
	...
	specificUpdate() {
		...
		if (!id) {
			id = this.quickxr.aframe.assetManager.updateCurrentAssetIdReturn()
			this.domElement.id = id
		}

		if (this.domElement.hasAttribute("vr")) {
			this.video360.object3D.visible = true
			this.video360.classList.add(this.quickxr.settings.interactiveTag)
			...
			if (rotation)
				this.video360.object3D.rotation.y =
					THREE.Math.degToRad(rotation)
			else this.video360.object3D.rotation.y = 0
		} else {
			this.video360.object3D.visible = false
			this.video360.classList.remove(this.quickxr.settings.interactiveTag)
			this.entity.object3D.visible = true
			this.entity.classList.add(this.quickxr.settings.interactiveTag)

...
export default class ContainerElement extends Element {
	constructor(quickxr, domElement, layer, textOnly = false) {
		...
		if (!this.textOnly) {
			...
			if (this.domElement == this.quickxr.container)
	...
	specificUpdate() {
		if (!this.textOnly) {
			...
			if (backgroundImage != "none") {
				...
				if (this.quickxr.settings.experimental) {
					if (
						backgroundImage != this.oldBackgroundImage ||
						(this.oldDomPosition &&
							!this.position.equalsDOMPosition(
								this.oldDomPosition
							))
					) {
						...
						if (!id) {
							...
							id =
								"html2canvas-" +
								this.quickxr.html2canvasIDcounter++
                    ...
                    #${id} > .vr-span {
                        display:block
							...
							if (this.expImageId)
								this.quickxr.aframe.assetManager.removeAsset(
									this.expImageId
								)
							this.expImageId =
								this.quickxr.aframe.assetManager.getAsset(
									canvas.toDataURL(),
									"img"
								)

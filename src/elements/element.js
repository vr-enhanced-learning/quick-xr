...
export default class Element {
	...
	paddingToMargin() {
		...
		if (this.domElement.textContent.trim()) {
			...
			const right =
				this.domElement.parentElement.element.style.paddingRight
	...
	update() {
		...
		if (clientRect.width == 0 || clientRect.height == 0) {
			...
			if (!this.domElement.classList.contains("vr-span"))
				this.entity.classList.remove(
					this.quickxr.settings.interactiveTag
				)
		...
		if (this.domElement.classList.contains("vr-span")) {
			...
			this.entity.classList.remove(this.quickxr.settings.interactiveTag)
		...
		if (
			this.quickxr.scroll.hasScroll &&
			this.domElement == this.quickxr.container &&
			this.quickxr.scroll.scrollBody
		)
			clientRect.height = this.quickxr.settings.scrollWindowHeight
		...
		if (this.visible) {
			...
			if (
				...
				if (
					(!this.domElement.classList.contains("vr-span") &&
						(this.mouseEventHandle.listeningForMouseEvents ||
							(this.domElement.tagName == "INPUT" &&
								this.domElement.type == "text"))) ||
					this.domElement == this.quickxr.container
				)
					this.entity.classList.add(
						this.quickxr.settings.interactiveTag
					)
			}
			...
			if (
				...
				this.needsStartingTransformSize
			) {
				this.entity.setAttribute("width", this.position.width)
				this.entity.setAttribute("height", this.position.height)
			}
	...
	updateTransform() {
		...
		if (transform == "none") transform = this.parentTransform
		else {
		...
		if (transform != "none") {
			...
			if (position.z != 0)
				this.position.depth =
					position.z * this.position.scalingFactor +
					this.position.startDepth
			...
			if (this.parentTransform == "none") {
				...
				const scaleY = Math.sqrt(
					elements[5] * elements[5] + elements[4] * elements[4]
				)
				...
				else this.entity.object3D.scale.set(scaleX, scaleY, 1)
			}
		}
	...
	updateBorder() {
		if (this.quickxr.settings.border) {
			...
			if (
				...
				this.entity.object3D.visible
			) {
				this.entity.setAttribute(
					"vr-border",
					`width:${borderWidth}; color:${this.style.borderTopColor};`
				)
	...
	rotateNormal(normal) {
		const normalMatrix = new THREE.Matrix3().getNormalMatrix(
			this.quickxr.aframe.container.object3D.matrixWorld
		)
	...
	getClippingContext() {
		...
		if (
			...
			this.domElement.parentNode.element &&
			this.domElement.parentNode.element.clippingContext
		)
			...
			if (
				...
				(this.quickxr.scroll.hasScroll &&
					this.domElement == this.quickxr.container) ||
				(this.quickxr.settings.forceClipping &&
					this.domElement == this.quickxr.container)
			) {
				// ignore if its svg
				if (this.domElement.tagName != "svg") {
					...
					clippingContext.bottom = new THREE.Plane(
						this.rotateNormal(new THREE.Vector3(0, 1, 0))
					) // normal up
					clippingContext.top = new THREE.Plane(
						this.rotateNormal(new THREE.Vector3(0, -1, 0))
					) // normal down
					clippingContext.left = new THREE.Plane(
						this.rotateNormal(new THREE.Vector3(1, 0, 0))
					) // normal right
					clippingContext.right = new THREE.Plane(
						this.rotateNormal(new THREE.Vector3(-1, 0, 0))
					) // normal left
	...
	setupClipping() {
		...
		if (clippingContext) {
			...
			if (
				!object3D ||
				!object3D.children ||
				!object3D.children.length > 0 ||
				!object3D.children[0].material
			) {
				return
			}
	...
	updateClipping() {
		...
		const bottomGlobal =
			this.quickxr.aframe.container.object3D.localToWorld(
				bottomLocal.clone()
			)
		const topGlobal = this.quickxr.aframe.container.object3D.localToWorld(
			topLocal.clone()
		)
		const leftGlobal = this.quickxr.aframe.container.object3D.localToWorld(
			leftLocal.clone()
		)
		const rightGlobal = this.quickxr.aframe.container.object3D.localToWorld(
			rightLocal.clone()
		)
		...
		if (this.quickxr.settings.showClipping) {
			if (!this.clippingPlaneHelpers) {
				...
				for (const plane of this.clippingContext.planes) {
					...
					const geometry = new THREE.PlaneGeometry(
						200 * this.position.scalingFactor,
						200 * this.position.scalingFactor
					) // no need same width and height as container because this is only for debugging
				...
				for (const helper of this.clippingPlaneHelpers) {
					this.quickxr.aframe.scene.object3D.add(helper)
				}
			}
			for (const mesh of this.clippingPlaneHelpers) {
				...
				const focalPoint = point
					.clone()
					.add(mesh.debugPlane.normal.clone().multiplyScalar(-1))
	...
	checkVisible() {
		if (this.clippingContext) {
			...
			if (
				this.position.bottom.y > container.position.top.y ||
				this.position.top.y < container.position.bottom.y
			) {
				this.visible = false
				this.entity.object3D.visible = false

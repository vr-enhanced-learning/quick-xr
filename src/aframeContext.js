...
export default class AframeContext {
	...
	createContainer(quickxrvr) {
		...
		// position container
		const width =
			parseFloat(window.getComputedStyle(quickxrvr.container).width) *
		...
		this.container.quickxrvr = quickxrvr
	}

	createSky() {
		// create sky if sky doesnt exist
		if (
			document.getElementsByTagName("a-sky").length == 0 &&
	...
	createControllers() {
		...
		if (!document.getElementById("mouseCursor")) {
			...
			cursor.setAttribute(
				"raycaster",
				`objects: .${this.settings.interactiveTag}, .collidable`
			)
		...
		// super hands 6DOF
		if (
			this.settings.createControllers &&
			...
			const superhands =
				"colliderEvent: raycaster-intersection; colliderEventProperty: els; colliderEndEvent:raycaster-intersection-cleared; colliderEndEventProperty: clearedEls; grabStartButtons: gripdown; grabEndButtons: gripup; stretchStartButtons: gripdown; stretchEndButtons: gripup"

			const leftHand = document.createElement("a-entity")
			leftHand.id = "leftHand"
			leftHand.setAttribute(
				"hand-controls",
				"hand:left; handModelStyle: highPoly; color: #ffcccc"
			)
			...
			rightHand.setAttribute(
				"hand-controls",
				"hand:right; handModelStyle: highPoly; color: #ffcccc"
			)

AFRAME.registerComponent("vr-border", {
	...
	init: function () {
		...
		this.el.element.quickxr.aframe.container.object3D.add(this.borderObject)

		//maybe clippingContext inside tick?
		if (this.el.element.clippingContext) {
			line.material.clippingPlanes =
				this.el.element.clippingContext.planes
	...
	tick: function () {
		if (this.running) {
			...
			this.borderObject.position.set(
				position.x,
				position.y,
				position.z + this.el.element.quickxr.settings.layerStep * 2
			)
			this.running = false
		}
	},
	remove: function () {
		this.el.element.quickxr.aframe.container.object3D.remove(
			this.borderObject
		)
	},
})
